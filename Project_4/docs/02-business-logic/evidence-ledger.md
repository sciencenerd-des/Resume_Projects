...
interface LedgerEntry {
  ...
  // Source tracking (3-LLM pipeline)
  ...
  expert_assessment?: string;    // Judge's expert verification notes
}
```
...
type ClaimType =
  ...
  | "policy"      // Policy or rule-based claims
  | "numeric"     // Numerical data or statistics
  | "definition"  // Definitions or terminology
  | "scientific"  // Scientific findings or research
  | "historical"  // Historical facts or events
  | "legal";      // Legal statements or interpretations

type Importance = "critical" | "material" | "minor";

type Verdict =
  | "supported"        // Evidence directly supports claim (confidence > 0.8)
  | "weak"             // Partial evidence (confidence 0.5-0.8)
  | "contradicted"     // Evidence conflicts with claim
  | "not_found"        // No relevant evidence found
  | "expert_verified"  // Verified by Judge LLM expertise
  | "conflict_flagged"; // Sources conflict, requires attention

// Source tag format for 3-LLM pipeline
type SourceTag =
  | `cite:${number}`   // Document citation (e.g., cite:1, cite:2)
  | "llm:writer"       // Generated by Writer LLM
  | "llm:skeptic"      // Challenged/verified by Skeptic LLM
  | "llm:judge";       // Verified by Judge LLM
...
│                                                             │
│  EXPERT VERIFIED (Teal)                                    │
│  ──────────────────────                                    │
│  • Verified by Judge LLM expertise                         │
│  • May not have direct document evidence                   │
│  • Includes expert assessment notes                        │
│  • High confidence from LLM domain knowledge               │
│                                                             │
│  WEAK (Amber)                                              │
│  ────────────                                              │
...
│                                                             │
│  CONFLICT FLAGGED (Orange)                                 │
│  ─────────────────────────                                 │
│  • Multiple sources provide conflicting information        │
│  • Requires manual review to determine truth               │
│  • Document which sources disagree                         │
│  • User must decide authoritative source                   │
│                                                             │
│  CONTRADICTED (Red)                                        │
│  ──────────────────                                        │
...
### 5.1 Citation Anchor Formats

The 3-LLM pipeline uses two types of citation anchors:

**Document Citations:** `[cite:N]` where N is the document reference number
```
In the response text:
"Employees are entitled to 15 days of annual leave [cite:1]."
                                                    └──────┘
                                                    Document citation
...
**LLM Knowledge Citations:** `[llm:source]` where source is writer/skeptic/judge
```
In the response text:
"This is a well-established industry standard [llm:writer]."
                                               └──────────┘
                                               LLM knowledge tag

"This claim requires additional verification [llm:skeptic]."
                                              └───────────┘
                                              Skeptic challenge

"Based on domain expertise, this is accurate [llm:judge]."
                                              └─────────┘
                                              Judge verification

In the ledger:
{
  claim_text: "This is a well-established industry standard",
  source_tag: "llm:writer",
  verdict: "expert_verified",
  expert_assessment: "Verified as common knowledge in HR practices"
}
```

### 5.2 Source Tag Color Coding

| Source Tag | Color | Description |
|------------|-------|-------------|
| `cite:N` | Emerald | Document-sourced claim |
| `llm:writer` | Blue | Generated by Writer LLM |
| `llm:skeptic` | Purple | Challenged by Skeptic LLM |
| `llm:judge` | Indigo | Verified by Judge LLM |

### 5.3 Chunk Hash Generation
...
CREATE INDEX idx_claims_source_tag ON claims(source_tag);
```
...
CREATE TABLE evidence_ledger (
  ...
  verdict TEXT NOT NULL CHECK (verdict IN (
    ...
    'expert_verified', 'conflict_flagged'
  )),
  ...
  -- Reasoning
  notes TEXT,
  expert_assessment TEXT,  -- Judge LLM verification notes
...
CREATE INDEX idx_ledger_source_tag ON evidence_ledger(source_tag);
```
